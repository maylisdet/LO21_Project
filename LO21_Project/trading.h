...
class Devise {
    ...
    Devise(const QString& c, const QString& = "", const QString& z = "");
    ~Devise() = default;
...
public:
    ...
    bool isIncreasing() const {if(getOpen()>getClose()){return false;}else{return true;};}
    double MecheSup() const;
    double MecheInf() const;
};

class EvolutionCours {
    friend class Modemanuel;
